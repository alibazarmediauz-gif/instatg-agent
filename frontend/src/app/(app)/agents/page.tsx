    ...
    const fetchAgents = async () => {
        try {
            ...
            const allAgents = [
                ...
                ...(voiceData.data || []).map((a: any) => ({ ...a, type: 'voice' }))
            ];
    ...
    if (loading) {
        return (
            <div className="page-container" style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', height: '100vh' }}>
                <div className="animate-pulse" style={{ color: 'var(--text-muted)' }}>Synchronizing Workforce...</div>
            </div>
        );
    }
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '320px 1fr', gap: 24, flex: 1, minHeight: 0 }}>
                ...
                <div className="card" style={{ padding: 0, overflowY: 'auto', display: 'flex', flexDirection: 'column' }}>
                    <div style={{ padding: '16px 20px', borderBottom: '1px solid var(--border)', background: 'var(--bg-elevated)', position: 'sticky', top: 0 }}>
                        <h3 style={{ fontSize: 14, fontWeight: 700 }}>Active Roster ({agents.length})</h3>
                    </div>
                            ...
                            >
                                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 6 }}>
                                    <span style={{ fontSize: 14, fontWeight: 700, display: 'flex', alignItems: 'center', gap: 8, color: 'var(--text-primary)' }}>
                                        <Bot size={16} color={agent.type === 'voice' ? 'var(--purple)' : 'var(--accent)'} />
                                        {agent.name}
                                    </span>
                                    <span className="status-dot" style={{ background: 'var(--success)' }} />
                                </div>
                                <div style={{ fontSize: 12, color: 'var(--text-secondary)', marginBottom: 12 }}>{agent.internal_role || (agent.type === 'voice' ? 'Voice AI Agent' : 'Chat Closer')}</div>
                                <div style={{ display: 'flex', gap: 6 }}>
                                    {agent.type === 'voice' && <span className="badge neutral" title="Voice Channel"><Mic size={10} /></span>}
                                    {agent.type === 'chat' && <span className="badge neutral" title="Chat Channel"><MessageSquare size={10} /></span>}
                                    <span className="badge neutral" style={{ fontSize: 10 }}>ID: {agent.id.slice(0, 8)}</span>
                                </div>
                            </div>
                        ))}
                    </div>
                        ...
                        <div style={{ padding: '24px 32px', borderBottom: '1px solid var(--border)', display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
                            <div>
                                <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 8 }}>
                                    <div style={{ width: 48, height: 48, borderRadius: 12, background: selectedAgent.type === 'voice' ? 'var(--purple)' : 'var(--accent)', display: 'flex', alignItems: 'center', justifyContent: 'center', boxShadow: '0 0 20px rgba(59, 130, 246, 0.3)' }}>
                                        <Bot size={24} color="white" />
                                    </div>
                                    <div>
                                        <h2 style={{ fontSize: 20, fontWeight: 800 }}>{selectedAgent.name}</h2>
                                        <span style={{ fontSize: 13, color: 'var(--text-muted)' }}>{selectedAgent.internal_role || 'Sales Associate'}</span>
                                    </div>
                                </div>
                                <div style={{ display: 'flex', gap: 12, marginTop: 16 }}>
                                    <span className="badge success" style={{ padding: '4px 10px' }}>Operational</span>
                                    <span className="badge outline" style={{ padding: '4px 10px', gap: 6 }}><Activity size={12} /> Live Workforce</span>
                                </div>
                            </div>
                            <div style={{ display: 'flex', gap: 12 }}>
                                <button className="btn btn-secondary"><Activity size={14} style={{ marginRight: 6 }} /> View Logs</button>
                                <Link href={`/studio?id=${selectedAgent.id}`} className="btn btn-primary" style={{ textDecoration: 'none' }}>
                                    <Settings size={14} style={{ marginRight: 6 }} /> Configure Persona
                                </Link>
                            </div>
                        </div>
                        ...
                        <div style={{ padding: '32px', display: 'flex', flexDirection: 'column', gap: 32 }}>
                            ...
                            <section>
                                ...
                                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16 }}>
                                    <div className="card" style={{ padding: '16px', border: selectedAgent.type === 'chat' ? '1px solid var(--accent)' : '1px solid var(--border)', background: selectedAgent.type === 'chat' ? 'rgba(59, 130, 246, 0.05)' : 'transparent' }}>
                                        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 12 }}>
                                            <div style={{ display: 'flex', alignItems: 'center', gap: 8, fontWeight: 700, fontSize: 14 }}>
                                                <MessageSquare size={16} color="var(--accent)" /> Chat Messaging
                                            </div>
                                            <div className={`switch ${selectedAgent.type === 'chat' ? 'active' : ''}`} />
                                        </div>
                                    ...
                                    <div className="card" style={{ padding: '16px', border: selectedAgent.type === 'voice' ? '1px solid var(--purple)' : '1px solid var(--border)', background: selectedAgent.type === 'voice' ? 'rgba(168, 85, 247, 0.05)' : 'transparent' }}>
                                        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 12 }}>
                                            ...
                                            <div className={`switch ${selectedAgent.type === 'voice' ? 'active' : ''}`} />
                                        </div>
                            ...
                            <section style={{ border: '1px solid var(--border)', padding: 24, borderRadius: 16, background: 'rgba(255,255,255,0.02)' }}>
                                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16 }}>
                                    <div>
                                        <h3 style={{ fontSize: 14, fontWeight: 700, marginBottom: 4 }}>Knowledge Workspace</h3>
                                        <p style={{ fontSize: 12, color: 'var(--text-secondary)' }}>Currently indexing manual sales documents and CRM context.</p>
                                    </div>
                                    <ChevronRight color="var(--text-muted)" />
                                </div>
                                <div style={{ display: 'flex', gap: 12 }}>
                                    <span className="badge outline" style={{ padding: '6px 12px' }}>RAG Engine Active</span>
                                    <span className="badge outline" style={{ padding: '6px 12px' }}>Manual Training Hub Connected</span>
                                </div>
                            </section>
